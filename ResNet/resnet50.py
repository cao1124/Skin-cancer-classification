...
class ResNet50(nn.Module):
    ...
    def __init__(self, classes_num):  # ResNet50仅传一个分类数目，将涉及的所有数据写死，具体数据可以参考下面的图片
        ...
        self.layer1_first = nn.Sequential(
            ...
            # layer1_first第一次卷积保持channel不变，和其他layer的first区别
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False),
            # layer1_first第二次卷积stride和其他layer_first的stride不同
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 256, kernel_size=1, stride=1, padding=0, bias=False),
            # layer1_first第三次卷积和其他layer一样，channel升4倍
        ...
        self.layer1_next = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 256, kernel_size=1, stride=1, padding=0, bias=False),
            # layer1_next的最后一次卷积负责将channel增加至可以与shortcut相加
            nn.BatchNorm2d(256)
        )
